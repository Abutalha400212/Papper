...
const ProductCard = ({ product}) => {
  const { name, img, price, bg, _id } = product;
  return (
    <div className={`mt-10 mx-auto text-center flex flex-col gap-3 p-5 ${bg} min-h-screen`}>
      ...
      <div className="flex justify-center items-center text-blue-800 font-serif">
        <Link to={`/details/${_id}`} className="link-hover text-xl">
          Learn More
          <ChevronRightIcon className="w-4 inline ml-0" />
        </Link>
        <Link
        to={`/cart/${_id}`}
